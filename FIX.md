...
## 2026-06-08 report-form添付ファイル上限修正

### 修正目的
report-formの資料ログ・スクリーンショット添付を最大5ファイルまでに制限する。
...
### 主な修正内容
- ファイル入力に `data-max-files="5"` を追加。
- 添付可能数の説明文を「最大5つまで」に更新。
- ファイル選択時と送信時の両方で、5ファイルを超えた場合にエラー表示し、送信されないようにした。

### 原則対応
- 混同リスク回避：フォームの劇中UI・注意書き・送信先に関する表記は維持。
- 没入感維持：フォームの設問、正解判定、送信後エンディング導線は変更なし。
- スマホ対応：既存のファイル入力UI・余白・ボタン配置は変更なし。

### 確認した項目
- JSエラー：report-formのインラインscriptを構文チェック。
- ページ表示：report-formがローカルHTTPで200応答することを確認。
- ファイル上限：`data-max-files="5"` と5件超過時のエラーメッセージが存在することを確認。

### 残っている懸念点
- ブラウザ実機でのファイル選択動作確認は、この環境にブラウザ実行ファイルがないため未実施。

## 2026-06-08 report-form / review-form Netlify Forms・公開可否対応

### 修正目的
report-form と review-form を Netlify Forms で検出・保存できる構造にし、review-form は既存Supabase投稿を維持したまま公開可否を扱えるようにする。
...
### 主な修正内容
- report-form の `<form>` 名を `report-form` に統一し、hidden `form-name` も `report-form` に変更。
- review-form（実体は `ending/feedback.html`）に `name="review-form"`、`method="POST"`、`data-netlify="true"`、hidden `form-name` を追加。
- review-form に「感想を公開しますか？」を追加し、初期値は「いいえ」に設定。
- Supabase insert に `is_public` を追加し、既存のSupabase URL / anon key / reviewsテーブルへの保存処理は維持。
- Supabase送信後に Netlify Forms へ `fetch("/")` で `review-form` の内容を送信する処理を追加。
- 送信後の自動遷移を削除し、完了メッセージとユーザーが押せる「みんなの感想を見る」ボタンだけを残した。
- みんなの感想ページは Supabase から `is_public = true` の感想のみ取得するように変更。
- Netlifyの静的公開先として `netlify.toml` に `publish = "."` を追加。

### 原則対応
- ヘッダー維持：固定FICTIONALヘッダーは変更なし。
- Supabase維持：Supabase接続情報と既存テーブル名 `reviews` は変更なし。
- 没入感維持：review-form の既存デザインを保ち、公開可否の文言だけを自然に追加。
- 導線維持：送信成功後は自動遷移せず、既存の「みんなの感想を見る」ボタンでのみ遷移。

### 確認した項目
- Netlify Forms：`report-form` と `review-form` の静的フォーム属性・hidden `form-name` を確認。
- 公開可否：review-form に `is_public` の「はい / いいえ」があり、初期値が「いいえ」であることを確認。
- Supabase：review-form の insert に `is_public` が含まれ、reviews一覧取得に `.eq("is_public", true)` があることを確認。
- 自動遷移：review-form から `window.location.assign` 等の自動遷移処理が削除されていることを確認。
- JSエラー：report-form、ending/feedback、reviews/script.js を構文チェック。
- ページ表示：report-form、ending/feedback、reviews がローカルHTTPで200応答することを確認。

### 残っている懸念点
- Supabase の `reviews` テーブルに `is_public` カラムが未作成の場合は、Supabase SQL Editorで `alter table reviews add column if not exists is_public boolean not null default false;` を実行する必要がある。
- Netlify Forms の管理画面検出と実送信確認は、Netlifyへデプロイ後に確認が必要。

## 2026-06-08

- 日付：2026-06-08
- 対象：ending
- 内容：自動検索フェーズ・劇中記事フェーズ内のFICTIONALヘッダー重複を解消。共通固定ヘッダーは維持し、フェーズ内表示は通常ラベル化。
- 確認：PC/スマホでヘッダーが一重表示であることを確認。
